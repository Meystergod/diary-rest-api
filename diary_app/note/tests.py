...
class NotesTests(APITestCase):
    ...
    def test_put_note_detail(self):
        # test update note (with permissions) (auth)
        self.client.login(email = 'user1@user.ru', password = 'user')
        self.client.credentials(HTTP_AUTHORIZATION = 'Token ' + self.test_user1_token.key)
        url_detail = reverse('note-detail', kwargs = {'pk': self.note_test1.id})
        data = {'content': 'updated', 'diary': self.diary_test1.id}
        response = self.client.put(url_detail, data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['content'], 'updated')
        self.client.logout()

    def test_error_put_note_detail(self):
        # test update note (non auth)
        url_detail = reverse('note-detail', kwargs = {'pk': self.note_test1.id})
        data = {'content': 'note', 'diary': self.diary_test1.id}
        response = self.client.put(url_detail, data)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        # test update note (without permissions)
        self.client.login(email = 'user1@user.ru', password = 'user')
        self.client.credentials(HTTP_AUTHORIZATION = 'Token ' + self.test_user1_token.key)
        url_detail = reverse('note-detail', kwargs = {'pk': self.note_test8.id})
        data = {'content': 'note updated', 'diary': self.diary_test1.id}
        response = self.client.put(url_detail, data)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertEqual(response.data['detail'], 'У вас недостаточно прав для выполнения данного действия.')
        self.client.logout()
        # test update note (invalid pk)
        self.client.login(email = 'user1@user.ru', password = 'user')
        self.client.credentials(HTTP_AUTHORIZATION = 'Token ' + self.test_user1_token.key)
        url_detail = reverse('note-detail', kwargs = {'pk': 0})
        data = {'content': 'note updated', 'diary': self.diary_test1.id}
        ...
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        self.client.logout()
        # test update note (invalid content)
        self.client.login(email = 'user1@user.ru', password = 'user')
        self.client.credentials(HTTP_AUTHORIZATION = 'Token ' + self.test_user1_token.key)
        url_detail = reverse('note-detail', kwargs = {'pk': self.note_test1.id})
        data = {'content': '', 'diary': self.diary_test1.id}
    ...
    def test_delete_note_detail(self):
        # test delete note (with permissions) (auth)
        self.client.login(email = 'user1@user.ru', password = 'user')
        self.client.credentials(HTTP_AUTHORIZATION = 'Token ' + self.test_user1_token.key)
        url_detail = reverse('note-detail', kwargs = {'pk': self.note_test1.id})
    ...
    def test_error_delete_note_detail(self):
        # test delete note (non auth)
        url_detail = reverse('note-detail', kwargs = {'pk': self.note_test1.id})
        response = self.client.delete(url_detail)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertEqual(response.data['detail'], 'Учетные данные не были предоставлены.')
        # test delete note (without permissions) (auth)
        self.client.login(email = 'user1@user.ru', password = 'user')
        self.client.credentials(HTTP_AUTHORIZATION = 'Token ' + self.test_user1_token.key)
        url_detail = reverse('note-detail', kwargs = {'pk': self.note_test8.id})
        response = self.client.delete(url_detail)
